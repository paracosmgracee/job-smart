...
# ══════════════════════════════════════════════════════════════════════════
    ...
    if st.button("Analyze Match", type="primary", use_container_width=True):
        if not resume_text or not jd_text:
            ...
            if not api_key or api_key == "your_groq_key_here":
                st.info("Add your Groq API key to enable AI analysis. Set GROQ_API_KEY in Streamlit secrets.")
            else:
                try:
                    import sys
    ...
    if st.button("Generate 90-Day Roadmap", use_container_width=True) and skills_input:
        api_key = _secret("GROQ_API_KEY")
        if not api_key or api_key == "your_groq_key_here":
            st.info("Add your Groq API key to Streamlit secrets to enable this feature.")
        else:
            try:
                import sys
                sys.path.insert(0, os.path.dirname(os.path.dirname(__file__)))
